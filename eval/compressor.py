...
class ContextualCompressor:
    """
    Wraps a set of already-retrieved chunks and compresses each one down to
    only the content that's actually relevant to the query, using the LLM.

    Why this matters: a retrieved chunk (e.g. 1000 chars) often contains a mix
    of relevant and irrelevant sentences - boilerplate, unrelated paragraphs,
    etc. Passing the full chunk to the generation LLM wastes context window
    and can dilute the signal. Compression extracts just the relevant part
    (or drops the chunk entirely if it turns out not to be relevant at all).

    This runs AFTER retrieval + reranking, on a small final set (e.g. top 5),
    since it costs one LLM call per chunk.
    """
    ...
    def compress(
        ...
        candidates: list of dicts with {"metadata": {"text": ...}, ...}
                    (output shape of HybridRetriever.search / CrossEncoderReranker.rerank)

        Returns a new list, same shape, but with:
          - metadata["text"] replaced by the compressed (relevant-only) text
          - metadata["original_text"] preserving the full original chunk
        Candidates with no relevant content at all are dropped from the result.
        """
        compressed_results = []
